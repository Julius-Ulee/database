...
(async () => {
  ...
  for (let i = 0; i < 20; i++) {
    ...
    for (let j = 0; j < warning_jumlah.length; j++) {
      warning.push($(data).eq(5).find('span').eq(j).text().replace('\t', ' ').trim());
    }
    result.push({
      waktu,
      lintang,
      ...
      kedalaman,
      wilayah,
      warning: warning.slice(0, warning.indexOf('')) // This ensures the slicing stops before any empty value
    });
  }
